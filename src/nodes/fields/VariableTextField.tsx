...
// Split stored value into variables and free text
...
// Combine variables + free text into stored value
...
export const VariableTextField: React.FC<VariableTextFieldProps> = ({ name, label, value, onChange, nodeId }) => {
  // Two-state: variables shown as chips, freeText shown in textarea
  ...
  // Sync combined value to store whenever vars or freeText change
  ...
  // When user selects a node from dropdown, add it as a variable chip
  ...
  // Handle textarea typing — only free text, trigger detection works on combined
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      const text = e.target.value;
      setFreeText(text);
      // Build combined for trigger detection
  ...
  // Sync from external value changes (e.g., store updates)
  useEffect(() => {
    if (value !== undefined) {
  ...
  // Build the raw value for VariableBadgeBar (it calls extractVariables internally)
    ...
    <label className="flex flex-col gap-1 relative">
      ...
      <div className="bg-background-secondary border border-secondary rounded-md shadow-xs
        ...
        <textarea
          ...
          placeholder={hasVars ? '' : 'Type {{ to reference nodes'}
        />
      </div>
